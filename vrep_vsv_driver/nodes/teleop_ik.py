        ...
        while not rospy.is_shutdown():
            if (rospy.rostime.get_time() - self.last_joy) < 0.5: 
                ...
                if (state == "Default") or (self.joy_value.buttons[self.move_button]):
                    ...
                    if (abs(dx)>1e-3) or (abs(dy)>1e-3) or (abs(dz)>1e-3):
                        new_x = x + dx
                        new_y = y + dy
                        new_z = z + dz
                        rospy.loginfo("Trying to move to %.2f %.2f %.2f from %.2f %.2f %.2f" \
                                % (new_x,new_y,new_z, x,y,z))
                        S = self.arm_ik.ik_xyz(new_x,new_y,new_z)
                        if S:
                    ...
                    # Probably wrong. 
                    (_,rot) = self.listener.lookupTransform('/%s/ArmTilt'%(self.name),
                            '/%s/ToolRotate'%(self.name), rospy.Time(0))
                    euler = euler_from_quaternion(rot)
                    command.position[index["ToolRotate"]] = euler[2]
            else:
                if not timeout:
                    timeout = True
